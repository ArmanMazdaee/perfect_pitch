...
def _batch_criterion(data, model, device):
    ...
    offsets_loss = _binary_cross_entropy_with_logits(
        prediction["offsets"], pianoroll["offsets"], pianoroll_weight["offsets"]
    )
    actives_loss = _binary_cross_entropy_with_logits(
        prediction["actives"], pianoroll["actives"], pianoroll_weight["actives"]
    )
    total_loss = onsets_loss + offsets_loss + actives_loss
    return {
